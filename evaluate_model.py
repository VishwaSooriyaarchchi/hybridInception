...
def main():
    ...
    check_args(args)
    set_seeds(2020)
    data_cfg = config.DataConfig(args["data_config"])   ## data file
    model_cfg = config.ModelConfig(args["model_config"]) ## model configuration file
    run_cfg   = config.RunConfig(args["run_config"], eval=True) ## running configuration file
